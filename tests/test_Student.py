...
def test_case_1():
    student = Student('沈小魚')
    for quest_num, s_ans in zip(range(1, 51), ['E','E','F','F','A','A','B','B','C','A','D','B','C','D','B','A','C','C','B','C','A','A','D','C','D','B','B','B','A','A','A','B','A','C','C','C','D','D','D','C','C','C','B','C','C','B','A','D','D','B']):
        student.answers.append(StudentAnswerVO(quest_num, s_ans))

    ans = {
        'name': '沈小魚',
        'error_analysis': {
            '形音義': ErrorAnalysisVO(correct=2, total=8),
            '詞語使用': ErrorAnalysisVO(correct=6, total=8),
            '成語運用': ErrorAnalysisVO(correct=6, total=8),
            '修辭技巧': ErrorAnalysisVO(correct=5, total=8),
            '國學常識': ErrorAnalysisVO(correct=5, total=8),
            '閱讀素養': ErrorAnalysisVO(correct=4, total=10),
        },
        'percentages': {
            '形音義': Decimal('25.0'),
            '詞語使用': Decimal('75.0'),
...
def test_calculate_score(test_case_1):
    ...
    for label, err in student.error_analysis.items():
        assert err.percentage == ans['percentages'][label], "Error analysis percentages do not match expected values"
        assert err.correct == ans['error_analysis'][label].correct, "Error analysis correct cnt do not match expected values"
        assert err.total == ans['error_analysis'][label].total, "Error analysis total cnt do not match expected values"

    student.get_figure()
    assert student.figure.name == ans['name'], f'Figure name error, should be {ans["name"]}'
    assert student.figure.values == list(ans['percentages'].values()), "Figure values not correct"
